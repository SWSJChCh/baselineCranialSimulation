'''
moveCell.py - Samuel Johnson - 07/11/2025
'''
...
'''
def moveCells(VEGFArray, cellList, filoNum, lenFilo, lenFiloMax, \
              xi, c0, cellRad, dx, dy, leadSpeed, folSpeed, \
    ...
    for i in cellList:
        ...
        # Initialise filoAge and angleList if they don't exist yet
        if not hasattr(i, "filoAge"):
            i.filoAge = 0
        ...
        if i.cellType == 'L':
            ...
            for filAngle in i.angleList:
                filList = []
                for j in range(int(cellRad), round(lenFilo + cellRad)):
            ...
            try:
                ...
                if ((maxGrad - cOld) / cOld) >= xi * math.sqrt(c0 / cOld):
                    if (cellRad < i.x + leadSpeed * dx * math.cos(maxAng) \
                        < length - cellRad and
                        cellRad < i.y + leadSpeed * dy * math.sin(maxAng) \
                            < width - cellRad and
                        not detectCollision(i, cellList, dx, dy, maxAng,
                                            expLen, cellRad, width, length,
                                            leadSpeed, folSpeed, c0)):
                        i.x += leadSpeed * dx * math.cos(maxAng)
                        i.y += leadSpeed * dy * math.sin(maxAng)
                        i.chainAngle = maxAng
                        moved = True
            ...
            if not moved:
                ...
                if (cellRad < i.x + leadSpeed * dx * math.cos(filAngle) \
                    < length - cellRad and
                    cellRad < i.y + leadSpeed * dy * math.sin(filAngle) \
                        < width - cellRad and
                    not detectCollision(i, cellList, dx, dy, filAngle,
                                            expLen, cellRad, width, length,
                                            leadSpeed, folSpeed, c0)):
                    i.x += leadSpeed * dx * math.cos(filAngle)
                    i.y += leadSpeed * dy * math.sin(filAngle)
        ...
        #Cell is follower
            ...
            if i.attachedTo == 0:
                ...
                for p in i.angleList:
                    ...
                    if detect == True:
                        ...
                        if cellRad < (i.x + folSpeed * dx * \
                                      math.cos(i.chainAngle)) < (length - cellRad) \
                                      and cellRad < (i.y + folSpeed * dy \
                                      * math.sin(i.chainAngle)) < (width - cellRad) and \
                                      not detectCollision(i, cellList, dx, dy, i.chainAngle,
                                            expLen, cellRad, width, length,
                                            leadSpeed, folSpeed, c0):
                            #Move in direction of filopodium
                            i.x += folSpeed * dx * math.cos(i.chainAngle)
                            i.y += folSpeed * dy * math.sin(i.chainAngle)
                            moved = True
                            break
                    ...
                    if cellRad < (i.x + folSpeed * dx * \
                        ...
                        * math.sin(filAngle)) < (width - cellRad) and \
                        not detectCollision(i, cellList, dx, dy, filAngle,
                                            ...
                                            leadSpeed, folSpeed, c0):
                        #Move in direction of filopodium
                        i.x += folSpeed * dx * math.cos(filAngle)
                        i.y += folSpeed * dy * math.sin(filAngle)
            ...
            #Cell is currently in chain
                ...
                if cellRad < (i.x + folSpeed * dx * math.cos(filAngle)) \
                    < (length - cellRad) and cellRad < (i.y + folSpeed * dy * \
                    math.sin(filAngle)) < (width - cellRad) and \
                       not detectCollision(i, cellList, dx, dy, filAngle,
                                            expLen, cellRad, width, length,
                                            leadSpeed, folSpeed, c0):
                ...
                elif cellRad < (i.x + folSpeed * dx * math.cos(randAng)) \
                    < (length - cellRad) and cellRad < (i.y + folSpeed * dy * \
                    math.sin(randAng)) < (width - cellRad) and \
                       not detectCollision(i, cellList, dx, dy, randAng,
                                            expLen, cellRad, width, length,
                                            leadSpeed, folSpeed, c0):
                    i.x += folSpeed * dx * math.cos(randAng)
                    i.y += folSpeed * dy * math.sin(randAng)
